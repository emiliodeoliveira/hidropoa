...
@Component({
  ...
  styleUrl: './monitor-cidade-campo-bom.component.css',
...
export class MonitorCidadeCampoBomComponent implements OnInit{
  ...
  public campoBomService = inject(SaladesituacaoServiceService)
  public campoBomData: GuaibaInfo[] = [];
...
ngOnInit(){
  this.campoBomService.campoBomData().subscribe(
    data => {
      this.parseData(data)
      this.getLastValues()    
    })
